...
import { Calendar, Music, Lightbulb, Users, User, LogOut, Home, Menu, X, Bell, Settings } from "lucide-react"
...
const bottomNavItems = [{ id: "profile", href: "/me/profile", label: "アカウント", icon: User }]

const currentUser = {
  isAdmin: true, // 管理者かどうか
}
  ...
  return (
    <>
      ...
      >
        ...
        <button
          onClick={() => setIsMobileMenuOpen(false)}
          className="absolute top-4 right-4 p-3 text-muted-foreground hover:text-foreground transition-colors"
          aria-label="メニューを閉じる"
        >
          <X className="h-6 w-6" />
        </button>
        ...
        <div className="px-8 pb-8 space-y-2">
          ...
          <Link
            href="/login"
            onClick={() => setIsMobileMenuOpen(false)}
            className="flex items-center gap-4 px-6 py-4 rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-all"
          >
