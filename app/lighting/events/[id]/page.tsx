...
type SongRow = {
  ...
  lighting_spot: number | null;
  lighting_strobe: number | null;
  lighting_moving: number | null;
...
const formatLighting = (value: number | null | undefined): string => {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const [eventRes, slotsRes, bandsRes] = await Promise.all([
        ...
        supabase
          .from("bands")
          .select("id, name, repertoire_status, general_note, lighting_note, lighting_total_min, stage_plot_data")
          .eq("event_id", eventId)
          .eq("band_type", "event"),
      ]);
      ...
      if (bandIds.length > 0) {
        const [membersRes, songsRes] = await Promise.all([
          supabase
            .from("band_members")
            .select("id, band_id, instrument, position_x, position_y, is_mc, profiles(display_name, real_name, part)")
            .in("band_id", bandIds),
          supabase
            .from("songs")
            .select("id, band_id, title, duration_sec, order_index, entry_type, lighting_spot, lighting_strobe, lighting_moving, lighting_color")
            .in("band_id", bandIds)
                    ...
                    {activeBand ? (
                      <div className="space-y-3">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {activeBand.lighting_total_min != null && (
                            <span className="text-[10px] text-muted-foreground ml-auto flex items-center gap-1">
                              <Lightbulb className="h-3 w-3" /> 照明打合せ {activeBand.lighting_total_min}分
                            </span>
                          )}
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          ...
                          <div className="border border-border rounded p-2 bg-card/60 min-w-0">
                            <div className="text-[10px] font-medium mb-0.5 text-amber-500">照明指示</div>
                            <div className="text-[10px] text-muted-foreground whitespace-pre-wrap line-clamp-3 break-words">{activeBand.lighting_note || "-"}</div>
                          </div>
                        </div>
                            ...
                            <div className="max-h-28 overflow-y-auto overflow-x-hidden">
                              <table className="w-full text-[10px]">
                                <thead className="text-[9px] text-muted-foreground sticky top-0 bg-card/60">
                                  <tr className="border-b border-border"><th className="py-0.5 text-left">パート</th><th className="py-0.5 text-left">名前</th><th className="py-0.5 text-left w-6">MC</th></tr>
                                </thead>
                                <tbody>
                                  {(membersByBand[activeBand.id] ?? []).length === 0 ? <tr><td colSpan={3} className="py-1 text-muted-foreground">なし</td></tr> : (membersByBand[activeBand.id] ?? []).map(m => {
                                    ...
                                    return (
                                      <tr key={m.id} className="border-b border-border/50">
                                        ...
                                        <td className="py-0.5">{m.is_mc ? "○" : ""}</td>
                                      </tr>
                                    );
                                  })}
                                </tbody>
                              ...
                              <table className="w-full text-[10px]">
                                <thead className="text-[9px] text-muted-foreground sticky top-0 bg-card/60">
                                  <tr className="border-b border-border">
                                    <th className="py-0.5 text-left w-4">#</th>
                                    <th className="py-0.5 text-left">曲名</th>
                                    <th className="py-0.5 text-left w-4" title="スポット">Sp</th>
                                    <th className="py-0.5 text-left w-4" title="ストロボ">St</th>
                                    <th className="py-0.5 text-left w-4" title="ムービング">Mv</th>
                                    <th className="py-0.5 text-left">色</th>
                                  </tr>
                                </thead>
                                <tbody>
                                  {(songsByBand[activeBand.id] ?? []).length === 0 ? <tr><td colSpan={6} className="py-1 text-muted-foreground">なし</td></tr> : (songsByBand[activeBand.id] ?? []).map((s, i) => (
                                    <tr key={s.id} className="border-b border-border/50">
                                      <td className="py-0.5 text-muted-foreground">{i + 1}</td>
                                      <td className="py-0.5 truncate max-w-16">{s.entry_type === "mc" ? <span className="text-muted-foreground">MC</span> : s.title || "未入力"}</td>
                                      <td className="py-0.5 text-muted-foreground">{formatLighting(s.lighting_spot)}</td>
                                      <td className="py-0.5 text-muted-foreground">{formatLighting(s.lighting_strobe)}</td>
                                      <td className="py-0.5 text-muted-foreground">{formatLighting(s.lighting_moving)}</td>
                                      <td className="py-0.5 text-muted-foreground truncate max-w-8">{s.lighting_color || "-"}</td>
                                    </tr>
                                  ))}
                                </tbody>
                              </table>
