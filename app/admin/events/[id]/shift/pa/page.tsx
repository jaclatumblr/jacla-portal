...
const isEligibleCrew = (crew?: string | null) => crew == null || crew === "User" || crew === "PA";
...
export default function AdminEventShiftPaPage() {
  ...
  const canEditRole = isAdmin || isPaLeader;
  ...
  const assignedPaIds = useMemo(() => {
    return new Set(eventStaff.filter((staff) => staff.can_pa).map((staff) => staff.profile_id));
  }, [eventStaff]);

  const helperOptions = useMemo(() => {
    return eligibleProfiles.filter(
      (profile) => !assignedPaIds.has(profile.id) && !eligiblePerformerIds.has(profile.id)
    );
  }, [assignedPaIds, eligiblePerformerIds, eligibleProfiles]);

  const paStaffOptions = useMemo(() => {
    return eventStaff.filter(
      (staff) => staff.can_pa && eligibleProfileIds.has(staff.profile_id)
    );
  ...
  const handleAddStaff = async (event: FormEvent) => {
    ...
    if (!eligibleProfileIds.has(staffForm.profileId)) {
      toast.error("PAシフトにはPAロールかUserのみ追加できます。");
      return;
    }
          ...
          <section className="flex-1 overflow-hidden py-6 md:py-8">
            <div className="container mx-auto px-4 sm:px-6 h-full flex flex-col gap-6 md:gap-8">
              ...
              <Card className="bg-card/60 border-border flex flex-col min-h-0">
                <CardHeader className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                  <div>
                    <CardTitle className="text-lg">PAスタッフ</CardTitle>
                    <CardDescription>
                      出演メンバーは自動追加されます。お手伝いはPAロールまたはUserのみ追加できます。
                    </CardDescription>
                  </div>
                  <Badge variant="outline" className="gap-1">
                    <Users className="w-4 h-4" />
                ...
                <CardContent className="space-y-4 max-h-[36vh] overflow-auto">
                  ...
                  {paStaffOptions.length === 0 ? (
                    <p className="text-sm text-muted-foreground">PAスタッフが未登録です。</p>
                  ) : (
